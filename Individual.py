...
class Individual:
    def __init__(self, problem):
        ...
        self.orederedGenes = {}  # Ordered individual' genes
    ...
    def AddOrderedGene(self, center, machine, gene):
        if center not in self.orederedGenes.keys():
            self.orederedGenes[center] = {}

        if machine not in self.orederedGenes[center].keys():
            self.orederedGenes[center][machine] = []

        self.orederedGenes[center][machine].append(gene)
    ...
    def __str__(self):
        ...
        for center in range(self.currentProblem.centerCount):
            ...
            for machine in range(self.currentProblem.resourcesCount[center]):
                ...
                if center > 0:
                    strBuilder.Append(" " * self.orederedGenes[center][machine][0].GetEndTime(center - 1))
                
                geneCount = 0
                for gene in self.orederedGenes[center][machine]:
                    ...
                    if geneCount == (len(self.orederedGenes[center][machine]) - 1):
        ...
        strBuilder.AppendLine("{}Cmax = {}{}".format("~" * 7, self.GetMakespan(), "~" * 7))
